...
def display_metrics():
    ...
    if df_data is not None:
        print(df_data)
        sum_revenue = df_data[df_data["Type"] == "TRANSFER"]["Value"].sum()
        sum_expense = df_data[df_data["Type"] == "CARD_PAYMENT"]["Value"].sum()
        profit = sum_revenue - sum_expense

    col_m1, col_m2, col_m3 = st.columns([NORMAL_SIZE,NORMAL_SIZE,NORMAL_SIZE])
    with col_m1:
        st.metric("Revenue", f"{millify(sum_revenue)}$")
    with col_m2:
        st.metric("Spent", f"{millify(sum_expense)}$")
    with col_m3:
        st.metric("Profit", f"{millify(profit)}$")
...
def add_dict_expenses(exp_name, exp_date, exp_type, exp_number):
    dbm.insert_data(exp_name, exp_date, exp_type, exp_number)
